...
func (ct *CronTrigger) NextFireTime(prev int64, location *time.Location) (int64, error) {
	...
	return parser.nextTime(prev, ct.fields, location)
}
...
func (parser *CronExpressionParser) nextTime(prev int64, fields []*CronField, location *time.Location) (nextTime int64, err error) {
	defer func() {
		if r := recover(); r != nil {
			switch x := r.(type) {
			case string:
	...
	tfmt := time.Unix(prev/int64(time.Second), 0).In(location).Format(readDateLayout)
	...
	dayOfMonth := parser.nextDay(intVal(days, strings.ToUpper(ttok[0])), fields[5], atoi(ttok[2]), fields[3])
	...
	ntime, err := time.ParseInLocation(writeDateLayout, nstr, location)
...
func buildCronField(tokens []string) ([]*CronField, error) {
	...
	fields[5], err = parseField(strings.ToUpper(tokens[5]), 0, 6, days)
	if err != nil {
		return nil, err
	}
